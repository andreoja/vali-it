...
public class AccountRepository {
    ...
    public void updateHistory(String account, BigDecimal depo, BigDecimal withraw, BigDecimal transferIn, BigDecimal transferOut){
        String sql = "UPDATE history SET depo = :depo WHERE account_nr = :accountNumber";
        Map<String, Object> paramMap = new HashMap<>();
        paramMap.put("depo", depo);
        paramMap.put("accountNumber", account);
        bankDatabase.update(sql, paramMap);
        sql = "UPDATE history SET withraw = :withraw WHERE account_nr = :accountNumber";
        Map<String, Object> paramMap1 = new HashMap<>();
        paramMap1.put("withraw", withraw);
        paramMap1.put("accountNumber", account);
        bankDatabase.update(sql, paramMap1);
        sql = "UPDATE history SET transferIn = :transferIn WHERE account_nr = :accountNumber";
        Map<String, Object> paramMap2 = new HashMap<>();
        paramMap2.put("transferIn", transferIn);
        paramMap2.put("accountNumber", account);
        bankDatabase.update(sql, paramMap2);
        sql = "UPDATE history SET transferOut = :transferOut WHERE account_nr = :accountNumber";
